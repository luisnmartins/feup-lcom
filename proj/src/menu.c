...
void draw_menu(int mode)
{
	...
	{
		print_buffer();
		fill_buffer(bmp->main_menu,0,0,0);
		update_menu_mouse();
...
{
	fill_buffer(bmp->cursor,x_pos_atual,y_pos_atual,1);
}
...
void draw_choose_snake(int mode)
{
	...
	{
		print_buffer();
		fill_buffer(bmp->choose_main,0,0,0);
	...
	{
		print_buffer();
		fill_buffer(bmp->choose_p1,0,0,0);
	...
	{
		print_buffer();
		fill_buffer(bmp->choose_p2,0,0,0);
		update_menu_mouse();
		return;
	}
}
...
void draw_time(int hour,int min, int seconds)
{
	int x_inicial =850;
	int y_inicial = 75;
	fill_buffer(bmp->numbers[(hour/10)],x_inicial,y_inicial,1);
	x_inicial += 40;
	fill_buffer(bmp->numbers[(hour%10)],x_inicial,y_inicial,1);
	x_inicial += 40;
	fill_buffer(bmp->numbers[10],x_inicial,y_inicial,1);
	x_inicial +=40;
	fill_buffer(bmp->numbers[(min/10)],x_inicial,y_inicial,1);
	x_inicial += 40;
	fill_buffer(bmp->numbers[(min%10)],x_inicial,y_inicial,1);
	x_inicial += 40;
	fill_buffer(bmp->numbers[10],x_inicial,y_inicial,1);
	x_inicial += 40;
	fill_buffer(bmp->numbers[(seconds/10)],x_inicial,y_inicial,1);
	x_inicial += 40;
	fill_buffer(bmp->numbers[(seconds%10)],x_inicial,y_inicial,1);
...
void draw_instructions(int mode)
{
	if (mode == 0)
	{
		fill_buffer(bmp->pausesymb,950,100,0);
		print_buffer();
		return ;
	}
	if (mode == 1)
	{
		fill_buffer(bmp->sp_inst,0,0,0);
		print_buffer();
		return;
	}
	if (mode == 2)
	{
		fill_buffer(bmp->kbc_inst,0,0,0);
		print_buffer();
		return;
	}
	if (mode == 3)
	{
		fill_buffer(bmp->mokb_inst,0,0,0);
		print_buffer();
			return;
	}
	return;
...
void show_points_sp(Snake *s1)
{
	...
	fill_buffer(bmp->player1,x_inicial-100,y-75,1);

	while(points >= 10)
	{
		fill_buffer(bmp->numbers[points%10],x_inicial,y,1);
		x_inicial -= 40;
		points = points /10;
	}
	fill_buffer(bmp->numbers[points%10],x_inicial,y,1);
...
void show_points_mp(Snake *s1, Snake *s2)
{
	...
	fill_buffer(bmp->player1,x1_in-100,y-75,1);
	fill_buffer(bmp->player2,x2_in-100,y-75,1);
	while(points_s1 >= 10)
		{
		fill_buffer(bmp->numbers[points_s1%10],x1_in,y,1);
			x1_in -= 40;
	...
	fill_buffer(bmp->numbers[points_s1%10],x1_in,y,1);
		while(points_s2 > 10)
				{
			fill_buffer(bmp->numbers[points_s2%10],x2_in,y,1);
					x2_in -= 40;
...
void draw_preview_snake(Snake *snake_preview, int counter, int body_count)
{
	...
	while(i<20)	{
		if(preview_matrix[i] != NULL)
		{
			...
			{
				fill_buffer(preview_matrix[i],20*i + 370,300,0);
			}else
			{
				fill_buffer(preview_matrix[i],20*i + 370,300,1);
			}
		}
		i++;
	}
