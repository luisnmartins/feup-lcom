...
void mouse_event_handler(unsigned long *packet_mouse) {
	...
	if (print_packet(3, packet_mouse, x, y, lb, rb) == 0) {
		...
		if (p == MENU_T) {
			if (*x >= 370 && *y >= 374 && *x <= (370 + 542) && *y <= (374 + 116)
					&& *lb == 1) {
				t = OPT_SP;
				check_game_status(&p, &t);
				return;
			}
			if (*x >= 370 && *y >= 522 && *x <= (370 + 538) && *y <= (522 + 112)
					&& *lb == 1) {
			...
			if (*x >= 370 && *y >= 658 && *x <= (370 + 542) && *y <= (658 + 116)
					&& *lb == 1) {
				t = OPT_EXIT;
				check_game_status(&p, &t);
				return;
		...
		if (p == MPMENU_T) {
			...
			if (*x >= 370 && *y >= 522 && *x <= (370 + 542) && *y <= (522 + 116)
					&& *lb == 1) {
				t = OPT_MOKB;
				check_game_status(&p, &t);
				return;
		...
		if (p == CHOOSE_SN_T) {
			if (t == OPT_KBC) {
				if (choose == 0) {
					...
					if (*x >= FIRST_ELEMENT_M && *y >= BODY_HEIGHT_M
							&& *x <= (FIRST_ELEMENT_M + SQUARE_WIDTH)
							&& *y <= (BODY_HEIGHT_M + SQUARE_HEIGHT)
							&& *lb == 1) {
						choose1 = 1;
						change_body(1);
						body_flag = 1;
					...
					if (*x >= SECOND_ELEMENT_M && *y >= BODY_HEIGHT_M
							&& *x <= (SECOND_ELEMENT_M + SQUARE_WIDTH)
							&& *y <= (BODY_HEIGHT_M + SQUARE_HEIGHT)
							&& *lb == 1) {
						choose1 = 2;
						change_body(2);
					...
					if (*x >= THIRD_ELEMENT_M && *y >= BODY_HEIGHT_M
							&& *x <= (THIRD_ELEMENT_M + SQUARE_WIDTH)
							&& *y <= (BODY_HEIGHT_M + SQUARE_HEIGHT)
							&& *lb == 1) {
						choose1 = 3;
						change_body(3);
			...
			if (t == OPT_SP || t == OPT_MOKB) {
				...
				if (*x >= FIRST_ELEMENT && *y >= BODY_HEIGHT
						&& *x <= (FIRST_ELEMENT + SQUARE_WIDTH)
						&& *y <= (BODY_HEIGHT + SQUARE_HEIGHT) && *lb == 1) {

					change_body(1);
					body_flag = 1;
					return;
				}
				if (*x >= SECOND_ELEMENT && *y >= BODY_HEIGHT
						&& *x <= (SECOND_ELEMENT + SQUARE_WIDTH)
						&& *y <= (BODY_HEIGHT + SQUARE_HEIGHT) && *lb == 1) {
					change_body(2);
					body_flag = 1;
					return;
				}
				if (*x >= THIRD_ELEMENT && *y >= BODY_HEIGHT
						&& *x <= (THIRD_ELEMENT + SQUARE_WIDTH)
						&& *y <= (BODY_HEIGHT + SQUARE_HEIGHT) && *lb == 1) {
				...
				if (*x >= FIRST_ELEMENT && *y >= HEAD_HEIGHT
						&& *x <= (FIRST_ELEMENT + SQUARE_WIDTH)
						&& *y <= (HEAD_HEIGHT + SQUARE_HEIGHT) && *lb == 1) {

					change_head(1);
					head_flag = 1;
					return;
				}
				if (*x >= SECOND_ELEMENT && *y >= HEAD_HEIGHT
						&& *x <= (SECOND_ELEMENT + SQUARE_WIDTH)
						&& *y <= (HEAD_HEIGHT + SQUARE_HEIGHT) && *lb == 1) {
					change_head(2);
					head_flag = 1;
				...
				if (*x >= THIRD_ELEMENT && *y >= HEAD_HEIGHT
						&& *x <= (THIRD_ELEMENT + SQUARE_WIDTH)
						&& *y <= (HEAD_HEIGHT + SQUARE_HEIGHT) && *lb == 1) {
