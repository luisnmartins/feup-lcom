...
void points_ingame_sp(Snake *s1) {
	...
	while (points >= 10) {
		drawbackground(bmp->numbers_score[points % 10], x_inicial, y,
				ALIGN_LEFT);
		x_inicial -= 40;
		points = points / 10;
	}
...
void points_ingame_mp(Snake *s1, Snake *s2) {
	...
	while (points_s1 >= 10) {
		drawbackground(bmp->numbers_score[points_s1 % 10], x1_in, y,
				ALIGN_LEFT);
		x1_in -= 40;
	...
	while (points_s2 >= 10) {
		drawbackground(bmp->numbers_score[points_s2 % 10], x2_in, y,
				ALIGN_LEFT);
		x2_in -= 40;
...
void clear_matrix() {
	memset(matrix_graphics, (int) NULL,
			sizeof(matrix_graphics[0][0]) * 64 * 64);
}
	...
	while (i_col < 64) {
		while (i_row < 64) {
			if (matrix_graphics[i_col][i_row] != NULL) {
				...
				if (matrix_graphics[i_col][i_row] == bmp->body
						|| matrix_graphics[i_col][i_row] == bmp->body2) {
					drawbackground(matrix_graphics[i_col][i_row], 20 * i_col,
							16 * i_row, ALIGN_LEFT);
				} else {
					drawBitmap(matrix_graphics[i_col][i_row], 20 * i_col,
							16 * i_row, ALIGN_LEFT);
...
void remove_snakes_matrix() {
	...
	while (i_col < 64) {
		while (i_row < 64) {
			if (matrix_graphics[i_col][i_row] == bmp->body
					|| matrix_graphics[i_col][i_row] == bmp->body2
					|| matrix_graphics[i_col][i_row] == bmp->cabeca1hd
					|| matrix_graphics[i_col][i_row] == bmp->cabeca1vb
					|| matrix_graphics[i_col][i_row] == bmp->cabeca1he
					|| matrix_graphics[i_col][i_row] == bmp->cabeca1vc
					|| matrix_graphics[i_col][i_row] == bmp->cabeca2hd
					|| matrix_graphics[i_col][i_row] == bmp->cabeca2vb
					|| matrix_graphics[i_col][i_row] == bmp->cabeca2he
...
int verify_colision_walls_bgmap(int col, int row, int mode) {
	if (mode == 1) {
		if (col < 4 || col >= 60 || row < 4 || row >= 60
				|| (row == 13 && col <= 20)
				|| (col == 12 && (row > 12 && row <= 30))
				|| (col == 17 && row >= 42)
				|| (row == 42 && (col > 9 && col <= 17))
				|| (col == 35 && (row >= 42)) || (row == 50 && (col >= 49))
				|| (col == 49 && (row <= 50 && row >= 33))
				|| (col == 43 && (row <= 21))
				|| (row == 21 && (col >= 35 && col <= 51))
				|| (row == 4 && (col <= 59 && col >= 54))) {
			return 1;
		}
		return 0;
		...
		if (col < 4 || col >= 60 || row < 4 || row >= 60
				|| (col == 12 && (row >= 26 && row <= 33))
				|| (row == 30 && col <= 12) || (col == 19 && row <= 13)
				|| (row == 13 && (col >= 15 && col <= 22))
				|| (col == 43 && row <= 21)
				|| (row == 21 && (col >= 43 && col <= 50))
				|| (col == 52 && (row >= 34 && row <= 42))
				|| (row == 34 && (col >= 52)) || (col == 35 && (row >= 42))
				|| (row == 48 && (col >= 27 && col <= 35))
				|| (row == 4 && (col <= 59 && col >= 54))
				|| (row == 4 && (col >= 4 && col <= 9))) {
			return 1;
		}
...
int update_matrix_snake(Snake *s1, int mouse) {
	...
	if (s1->head->row > 63 || s1->head->row < 0 || s1->head->col > 63
			|| s1->head->col < 0) {
		return 1;
	}
	...
	if (seg1 == s1->head) {
		...
		if (matrix_graphics[seg1->col][seg1->row] == bmp->body
				|| matrix_graphics[seg1->col][seg1->row] == bmp->body2
				|| matrix_graphics[seg1->col][seg1->row] == bmp->bomb) //TODO need to add other colisions
						{
...
int update_matrix_snakemp(Snake *s1, Snake *s2, int *snake1_alive,
		int *snake2_alive) {
	remove_snakes_matrix();

	if (s1->head->row > 63 || s1->head->row < 0 || s1->head->col > 63
			|| s1->head->col < 0 || s2->head->row > 63 || s2->head->row < 0
			|| s2->head->col > 63 || s2->head->col < 0) {
		return 1;
	}
	segment_snake *seg1 = s1->tail;
	...
	if (*snake2_alive == 0) {
		...
		if (seg2 == s2->head) {
			...
			if (matrix_graphics[seg2->col][seg2->row] == bmp->body
					|| matrix_graphics[seg2->col][seg2->row] == bmp->body2
					|| matrix_graphics[seg2->col][seg2->row] == bmp->bomb) //TODO need to add other colisions
							{
...
int update_matrix_objects(Game_object *obj, Snake *s1) {
	...
	if (matrix_graphics[obj->col][obj->row] != NULL)
		return 1;
	if ((abs(obj->row - s1->head->row) < 2 || abs(obj->col - s1->head->col) < 2)
			|| (obj->row == (s1->size - 1) || obj->col == (s1->size - 1)
					|| obj->row + (s1->size - 1) == 63
					|| obj->col + (s1->size - 1) == 63)) {
		return 1;
	}
...
int update_matrix_objects_2_snakes(Game_object *obj, Snake *s1, Snake *s2) {
	...
	if ((abs(obj->row - s2->head->row) < 2 || abs(obj->col - s2->head->col) < 2)
			|| (obj->row == (s2->size - 1) || obj->col == (s2->size - 1)
					|| obj->row + (s2->size - 1) == 63
					|| obj->col + (s2->size - 1) == 63)) {
		return 1;
	}
...
int add_fruit_matrix(int x, int y, Snake *s1) {
	...
	if (x > 1280 || x < 0 || y > 1024 || y < 0)
		...
		if (((abs(obj->row - s1->head->row) < 2
				&& abs(obj->col - s1->head->col) < 2))
				|| (obj->row == (s1->size - 1) || obj->col == (s1->size - 1)
						|| obj->row + (s1->size - 1) == 63
						|| obj->col + (s1->size - 1) == 63)) {
			return 1;
